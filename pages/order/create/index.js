...
Page({
  ...
  onLoad: function(t) {
    ...
    console.log(e.globalData.is_users)
    if (t.is_users){
      if (t.is_users == 2) {
        e.globalData.is_users = 2
        this.setData({
          type_isop: t.is_users,
          num_tatal: t.tatal
        })
      }
      else {
      ...
      if (e.globalData.is_users == 2) {
        this.setData({
          type_isop: e.globalData.is_users,
          num_tatal: t.tatal
        })
      } else {
        this.setData({
    ...
    console.log(this.data.type_isop)
    var i = this,
      r = [];
    if (t.goods) {
  ...
  submit: function() {
    getApp().globalData.rido_zen = this.data.ridos
    console.log(getApp().globalData.rido_zen)
    var t = this.data,
      e = this,
      i = this.data.diyform,
      d = t.giftid;
    if (0 == this.data.goods.giftid && 1 == this.data.goods.gifts.length && (d = this.data.goods.gifts[0].id), !t.submit && r.verify(this, i)) {
      ...
      if (t.type_isop == 1) {
        e.setData({
          opsaddss: t.list.address ? t.list.address.id : 0,
          ridos:''
        })
      }
      if (t.type_isop == 2) {
      ...
      if (t.list.storeInfo && (o.carrierid = t.list.storeInfo.id), 1 == t.data.dispatchtype || t.list.isvirtual || t.list.isverify) {
        ...
        o.addressid = 0;
      } else if (!o.addressid && !t.list.isonlyverifygoods && t.type_isop==1) return void a.alert("地址没有选择!");
      e.setData({
        submit: !0
