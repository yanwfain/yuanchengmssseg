...
Page((e = {
  ...
  onLoad: function(t) {
    // wx.setStorageSync("nid", t.nid)
    // wx.setStorageSync("id", t.id)
    // wx.setStorageSync("user_openid", t.user_openid)
    if (t.id_types){
      wx.showToast({
        title: '已经领取过了！',
      })
    }
    if (wx.getStorageSync("nid")) {
      // wx.showToast({
      //   title: '未授权',
      //   icon:'none'
      // })
      this.setData({
        type_status_isshow:true
      })
      setTimeout(function(){
        wx.setStorageSync("nid",'')
      },60000)
    }
    t = t || {};
    var a = this;
    ...
    setTimeout(function(){
      if (wx.getStorageSync("id")) {
        ...
        // wx.showToast({
        //   title: '真的' + wx.getStorageSync("id"),
        // })
        // wx.navigateTo({
        //   url: "../order/zenpin/index?id=" + wx.getStorageSync("id", t.id) + '&nid=' + wx.getStorageSync("nid", t.nid) + '&user_openid=' + wx.getStorageSync("user_openid", t.user_openid)
        // })
        // wx.setStorageSync("id", '')
        // wx.setStorageSync("nid", '')
        // wx.setStorageSync("user_openid", '')
      } else {
        // wx.showToast({
        //   title: '假的' + wx.getStorageSync("id"),
        // })
      }
    },1000)
